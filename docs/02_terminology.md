...
*   **Graph Surgery:**
    ...
    *   **Constraint:** This operation is performed *only* on the backend (`apps/backend`) using the `onnx` library. Shape inference must be run immediately afterward. For detailed rules, see [AI Coding Rules (04_convention.md)](file:///Users/twyou/Projects/string_lights/docs/04_convention.md).
...
*   **Web Inference:**
    ...
    *   For the data flow pipeline, see [Architecture & Data Flow (03_architecture.md)](file:///Users/twyou/Projects/string_lights/docs/03_architecture.md).
...
*   **WebGPU Fallback & WASM Mode Badge:**
    ...
    *   **UX Requirement:** When running in WASM mode, a warning status badge reading "WASM 모드 동작 중 - 성능 저하 가능" must be displayed on screen to inform users of potential performance lag.
