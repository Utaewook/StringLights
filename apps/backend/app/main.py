...
async def perform_surgery(
    ...
    async with surgery_semaphore:
        try:
            ...
            # 3. Perform Graph Surgery or copy depending on parameters
            if perform_surgery:
                try:
                    run_graph_surgery(input_onnx_path, output_onnx_path)
            ...
            # Register background task to clean up the temporary directory after sending response
            ...
            # Clean up immediately if an error occurs before returning the FileResponse
